...
temp_board = Board.Board("White")

def displayCurrentStatusBoard(temp_board):
    for row in range(gc.BOX_COUNT_PER_SIDE):
        for col in range(gc.BOX_COUNT_PER_SIDE):
            cur = temp_board.getPiece(row,col)
            if cur != '.':
...
def displayMoves(moves):
    dot_image = image.load('Other/dot.png',(gc.BOX_SIDE_LENGTH,gc.BOX_SIDE_LENGTH))
    dot_image = transform.scale(dot_image,)
    for x,y in moves:
        starting_x_of_box = y*gc.BOX_SIDE_LENGTH + gc.SCREEN_MARGIN_SIDE
        starting_y_of_box = x*gc.BOX_SIDE_LENGTH + gc.SCREEN_MARGIN_TOP
